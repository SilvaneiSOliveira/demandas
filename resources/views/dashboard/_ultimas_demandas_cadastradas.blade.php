@forelse($ultimasDemandas as $demanda)
    <tr class="odd:bg-gray-50 even:bg-white hover:bg-gray-100 border-t text-sm text-gray-800" data-id="{{ $demanda->id }}">
        ...
        <td class="px-4 py-2">{{ $demanda->cliente->tipo_suporte ?? '-' }}</td>
        <td class="px-4 py-2">
        ...
        <td class="px-4 py-2">
            {{ $demanda->atendente ? explode(' ', $demanda->atendente)[0] : '-' }}
        </td>
    </tr>
@empty
    <tr>
